...
class Encoder(torch.nn.Module):
    def __init__(self, n_feature, n_hidden, n_layer):
        ...
        self.low_embedding_x = nn.Linear(n_feature, n_hidden)                
        self.high_embedding_x = nn.Linear(n_feature, n_hidden)
    ...
    def forward(self, batch_data, mask = None):
        ...
        for i in range(self.n_cell):
            self.embedded_x = self.low_embedding_x(self.batch_data[:, i, :, :])                    
            _low_node = self.low_attention(self.embedded_x) # [batch_size, n_node, n_embedding]
        ...
        self.high_node = self.high_embedding_x(self.high_node)        
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, node_context, cell_context, original_data, high_mask, low_mask, low_decoder):                
        ...
        node_log_prob = torch.reshape(node_log_prob, [-1, 1]).squeeze(1)        
        node_action = torch.reshape(node_action, [-1, self.n_node])
        node_reward = torch.reshape(node_reward, [-1, 1]).squeeze(1)
