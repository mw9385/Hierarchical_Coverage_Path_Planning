...
parser.add_argument('--steps', default= 10000, help="number of epochs")
# hyperparameters
...
parser.add_argument('--n_layer', default=4, help='number of visiting nodes')
...
# evaluation
parser.add_argument('--log_interval', default=2000, help="store model at every epoch")
parser.add_argument('--eval_interval', default=2000, help='update frequency')
# paths
parser.add_argument('--log_dir', default='./log/V_no_hierarchy_3', type=str, help='directory for the tensorboard')
parser.add_argument('--file_name', default='HCPP_V_no_hierarchy_3', help='file directory')
parser.add_argument('--test_file_name', default='test_performance/V_no_hierarchy_3/', help='test_file directory')
args = parser.parse_args()
# save args
with open('./model/HCPP/model_V_no_hierarchy_3.txt', 'w') as f:
...
# optimizer = torch.optim.Adam(low_model.parameters(), lr = learning_rate)
all_params = list(low_model.parameters()) + list(high_model.parameters())
optimizer = torch.optim.Adam(all_params, lr = learning_rate)
# optimizer = torch.optim.Adam([
#                             {'params':low_model.parameters()},
#                             {'params':high_model.parameters(), 'lr':learning_rate}], lr=learning_rate)

# visualization code
def calculate_distance(input_node, output_node):
    return torch.norm(input_node - output_node, dim=0)

def plot_tsp(sample, high_action, low_action, test_file_name, global_step):    
    plt.figure()   
    plt.grid()
    high_action = high_action.cpu().numpy()    

    R = 0
    for i, high_index in enumerate(high_action):                        
        current_cell = sample[high_index]   
        low_index = low_action[high_index]        
        low_index = low_index.cpu().numpy()     
        plt.scatter(current_cell[:,0], current_cell[:,1], s=5)
        for j, low_sample in enumerate(low_index):
            points = current_cell[low_sample]                

            if i==0 and j==0:                
                # set the depot points
                plt.text(points[0], points[1], 'depot', fontsize=10, label='depot') 
            else:
                plt.plot([_points[0], points[0]], [_points[1], points[1]], linewidth=0.8 , color='black')
                plt.text(points[0], points[1], j, fontsize=5)
    ...
    for epoch in range(n_epoch):        
        for step in tqdm(range(steps)):    
            ...
            if global_step !=0 and global_step % eval_interval == 0:
                ...
                test_low_mask = torch.zeros([B, n_nodes], dtype= torch.int64).cuda()
                ...
                # test_total_cost = test_low_cost.mean()                
                ...
                # reshape the test low action size
                test_low_action = torch.reshape(test_low_action, [-1, n_cells, n_nodes])
                sample_low_action = test_low_action[random_batch_index]    
                
                plot_tsp(sample=sample, high_action=sample_high_action, low_action=sample_low_action, test_file_name=test_file_name, global_step = global_step)
